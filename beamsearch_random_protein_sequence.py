...
@torch.no_grad()
def sample_pt(model, path, vocabulary, proVoc, smiMaxLen, proMaxLen, device, sampleTimes, protein_seq, protein_coor):
    model.eval()
    ...
    protein_coor = torch.as_tensor([protein_coor]).to(device)

    sl = length - 1
    out = model(src, tgt, smiMask, proMask, tgt_mask, protein_coor)[:, sl, :]
    out = out.tolist()[0]
    pr = np.exp(out) / np.sum(np.exp(out))
    prList = np.random.multinomial(1, pr, sampleTimes)
