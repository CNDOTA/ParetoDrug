...
class MyDataset(Dataset):
    ...
    def __init__(self, data):
        proIndices, smiIndices, labelIndices, proMask, smiMask, proCoorArr = data
    ...
    def __getitem__(self, idx):
        ...
        return self.x[idx], self.y[idx], self.label[idx], np.array(proMask).astype(int), np.array(smiMask).astype(int), self.proCoorArr[idx]
...
def prepareData(config, orign):
    ...
    data = pd.read_csv('./data/train-val-data.tsv', sep='\t')  # data.columns: 'protein', 'smiles', 'pdbids'
    ...
    protein_coordination_dic = pkl.load(open('./data/protein_coordination.pkl', 'rb'))
    proCoorArr = []
    for proseq in tqdm(data['protein']):
        proCoorArr.append(protein_coordination_dic[proseq])
    proCoorArr = np.array(proCoorArr)
    
    logger.info('prepare %s smiles' % orign)
    if os.path.exists('./data/{}_smiles.pkl'.format(orign)):
        smiIndices, labelIndices, smiMask = pkl.load(open('./data/{}_smiles.pkl'.format(orign), 'rb'))
    else:
        smiIndices, labelIndices, smiMask = fetchIndices(smiArr, config.smiVoc, config.smiMaxLen)
        pkl.dump([smiIndices, labelIndices, smiMask], open('./data/{}_smiles.pkl'.format(orign), 'wb'))
    
    logger.info('prepare %s proteins' % orign)
    if os.path.exists('./data/{}_proteins.pkl'.format(orign)):
        proIndices, _, proMask = pkl.load(open('./data/{}_proteins.pkl'.format(orign), 'rb'))
    else:
        proIndices, _, proMask = fetchIndices(proArr, config.proVoc, config.proMaxLen)
        pkl.dump([proIndices, _, proMask], open('./data/{}_proteins.pkl'.format(orign), 'wb'))

    # proIndices: (192712, 1001), [ 1 12  2 ... 22 22 22]
    return proIndices, smiIndices, labelIndices, proMask, smiMask, proCoorArr
