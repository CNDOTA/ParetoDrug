...
moleculeParetoFront = {}  # used to store the docking information, score_vector = [binding1, binding2, ...]
        ...
        if mols and len(smileK) < smiMaxLen:  # valid molecule
            ...
            if smileK in infoma:
                ...
                if -500 not in score_vector:
                    logger.success("{} with Docking scores {}".format(smileK, [round(i, 5) for i in score_vector]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-q', type=str, default='HIV', help='multi-target data folder')
    ...
    SCOREDIM = len(test_pro_pdb_list) + len(test_anti_pdb_list)
    ...
    shutil.copyfile('mt_pareto_mcts.py', os.path.join(experimentId, resFolder) + '/mt_pareto_mcts.py')
    ...
    if mol:
        ...
        mol_dic['alpha_molecule'] = (alphaSmi, [np.asarray(score_vector).flatten().tolist(), qed(mol), MolLogP(mol), sascorer.calculateScore(mol), npscorer.scoreMol(mol, fscore)])
    else:
        print('Invalid molecule')

    pkl.dump(mol_dic, open('./experiment/{}_{}_st_{}.pkl'.format(args.q, args.t, args.st), 'wb'))
